...
const Offer = () => {
  return (
    <section className="relative w-full py-12 text-white text-center bg-blue overflow-hidden">
      ...
      {/* Image Sequence */}
      <div className="relative flex justify-center space-x-3 mb-8 z-10">
      ...
      {/* Step Explanations */}
      <div className="grid grid-cols-2 gap-8 px-12 text-left mb-10 relative z-10">
        {spaSteps.map((step, index) => (
          <div key={index} className="relative p-4 ml-9 mr-32">
            <span className="absolute top-0 left-0 text-9xl opacity-20 text-gold -z-10 font-serif">
              {index + 1}
            </span>
            <h3 className="text-2xl text-gold font-serif">{step.title}</h3>
